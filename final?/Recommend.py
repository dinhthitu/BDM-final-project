    ...
    def load_data(self):
        ...
        try:
            ...
            if unmatched_categories:
                print(f"⚠️ Warning: The following categories in {self.country_code}_category_id_to_name.json "
                      f"are not in {self.country_code}_category_stats.csv or have no data: {unmatched_categories}")
            
            self.category_names = {k: v for k, v in self.category_names.items() if k in valid_categories}
            
            # Debug: Print filtered categories
            print(f"DEBUG: Filtered category_names for {self.country_code}: {self.category_names}")
            print(f"DEBUG: Categories in category_stats: {list(category_stats_names)}")
            print(f"DEBUG: Valid category IDs in df: {list(valid_category_ids)}")
    ...
    def generate_eda_recommendations(self, insights, top_features, category_data):
        ...
        if insights['time_to_trend']:
            ...
            if time_stats['std'] is not None:
                ...
                recommendations.append(
                    f"• Aim to trend within {time_stats['median']:.1f} days (limited data, range not available)."
                )
